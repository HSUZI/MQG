...
import com.qingqu.wc.maqi.bean.ShopCartBean;
...
//        Glide.with(context).load(data.get(position).getDefaultPic()).into(holder.ivShopCartClothPic);
        holder.ivShopCartClothPic.setImageResource(data.get(position).getDefaultPic());
        if (position > 0) {
        ...
        holder.tvShopCartClothColor.setText("贈送點數：" + data.get(position).getColor());
        holder.tvShu.setText("X"+data.get(position).getCount() + "");
            ...
            holder.tv_bianji.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    if(data.get(position).getIsFirst() == 1) {
                        data.get(position).setShopBj(!data.get(position).getShopBj());
                        for(int i = 0;i < data.size();i++){
                            if(data.get(i).getShopId() == data.get(position).getShopId()){
//                               holder.ivShopCartClothDelete.setVisibility(View.VISIBLE);
                                data.get(i).setShopBjss(data.get(position).getShopBj());
        ...
        holder.tvShopCartClothPrice.setText("NT " + data.get(position).getPrice());
        ...
        if (data.get(position).getShopBj()){
            holder.tv_bianji.setText("完成");
        }else{
            holder.tv_bianji.setText("編輯");
        }
        if (data.get(position).getShopBjss()){
            holder.ivShopCartClothDelete.setVisibility(View.VISIBLE);
        }else{
            holder.ivShopCartClothDelete.setVisibility(View.GONE);
        }
        if(data.get(position).getIsSelect()){
            holder.ivShopCartClothSel.setImageDrawable(context.getResources().getDrawable(R.mipmap.shopcart_selected));
        }else {
            holder.ivShopCartClothSel.setImageDrawable(context.getResources().getDrawable(R.mipmap.quanxuan));
        }
        ...
        if(data.get(position).getIsShopSelect()){
            ...
            holder.ivShopCartShopSel.setImageDrawable(context.getResources().getDrawable(R.mipmap.quanxuan));
        }
    ...
    class MyViewHolder extends RecyclerView.ViewHolder
    {
        ...
        public MyViewHolder(View view)
        {
            ...
            tvShu = (TextView) view.findViewById(R.id.shu);
